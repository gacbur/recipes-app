...
const PinnedRecipe: React.FC<recipe> = ({ id, title, image }) => {
    ...
    const handleAddToDone = (single_recipe_item: recipe) => {
        dispatch(addToDone(single_recipe_item))
        dispatch(removeFromPinned(single_recipe_item.id))
    }

    const handleAddToFavorite = (single_recipe_item: recipe) => {
    ...
    return (
        <div className="pinned-recipe">
            ...
            <div className="pinned-recipe__text">
                <button
                    onClick={() => handleAddToFavorite(single_recipe_item)}
                >
                ...
                <button
                    onClick={() => handleAddToDone(single_recipe_item)}
                ><ImCheckmark2 /></button>
            </div>
        </div>
    )
